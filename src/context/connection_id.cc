...
LocalConnectionContext::LocalConnectionContext(uint64_t sequence,
                                               utils::timepoint current)
    : localID(ConnectionIDGenerator::Get().Generate()),
      sequence{sequence},
      createTime(current) {}

const ConnectionID& LocalConnectionContext::ID() const { return this->localID; }

bool LocalConnectionContext::Valid() { return this->localID.Valid(); }

RemoteConnectionContext::RemoteConnectionContext(const ConnectionID& remoteID,
                                                 uint64_t sequence)
    : remoteID(remoteID), sequence(sequence) {}

bool RemoteConnectionContext::Valid() { return this->remoteID.Valid(); }

const ConnectionID& RemoteConnectionContext::ID() const {
