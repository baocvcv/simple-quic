#ifndef _THQUIC_UTILS_CHUNKSTREAM_H_
#define _THQUIC_UTILS_CHUNKSTREAM_H_
   ...
   public:
    ...
    void AddChunk(uint64_t offset, std::unique_ptr<uint8_t[]> buf, size_t len,
        ...
        if (std::distance(right, left) > 0) {
            if ((maximumContinuousChunk != this->chunks.end()) &&
                std::distance(maximumContinuousChunk, left) >= 0) {
    ...
    void Logout() {
        ...
        for (auto& chunk : chunks) {
            chunksStr += "(" + std::to_string(chunk.MinimumOffset()) + " : " +
                         std::to_string(chunk.MaximumOffset()) + ")";
        }
        logger::warn("chunks: {}", chunksStr);
    }
