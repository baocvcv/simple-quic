...
static uint64_t encodeUpperBound[4] = {0x3F, 0x3F'FF, 0x3F'FF'FF'FF,
                                       0x3F'FF'FF'FF'FF'FF'FF'FF};
static EncodeParam encodeParams[4] = {
...
size_t encodeVarIntLen(uint64_t value) {
    uint8_t encodeIndex = std::upper_bound(std::begin(encodeUpperBound),
                                           std::end(encodeUpperBound), value) -
                          std::begin(encodeUpperBound);
    return encodeParams[encodeIndex].second;
}
...
int encodeVarInt(ByteStream& stream, uint64_t value) {
    ...
    uint8_t encodeFlag = encodeParams[encodeIndex].first;
    uint8_t encodeSize = encodeParams[encodeIndex].second;
