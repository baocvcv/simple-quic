   ...
   public:
    ...
    int ConnectionReady(uint64_t connection) {
        ...
        for (int i = 0; i < 7; i++) {
            uint64_t stream = client.CreateStream(connection, true);
            thquic::utils::logger::warn(
                "[APP] connection {} ready, creating stream {}", connection, stream);
            Send(connection, stream, "GET DIGEST");
            client.SetStreamDataReadyCallback(
                connection, stream,
                [&](uint64_t seq, uint64_t stream, std::unique_ptr<uint8_t[]> buf,
                size_t len, bool Fin) -> int {
                    thquic::utils::logger::debug("Hi from stream {}", stream);
                    auto digest =
                        *reinterpret_cast<uint64_t*>(temporaryBuffer.get());
                    thquic::utils::logger::info(
                        "[App] stream {} receive digest {:x}, request file", stream, digest);
                    return 0;
                }
            );
